...
function run_ocr($file_path, $kode_tagihan = '', $jumlah_tagihan = 0) {
    ...
    if ($output) {
        // Coba decode sebagai JSON (output baru dari ocr.py)
        $json = json_decode($output, true);
        if ($json && is_array($json)) {
            // Output JSON valid dari Python
            $result['text'] = $json['text'] ?? '';
            $result['jumlah'] = isset($json['amount']) ? intval($json['amount']) : 0;
            $result['tanggal'] = $json['date'] ?? '';
            $result['kode'] = $json['code'] ?? '';
            $result['confidence'] = isset($json['confidence']) ? floatval($json['confidence']) : 0.0;
        } else {
            // Fallback parsing lama jika bukan JSON
            $result['text'] = trim($output);
            $lines = explode("\n", trim($output));
            ...
            // Jika gagal parsing manual, coba pakai fallback
            if ($result['jumlah'] == 0) {
    ...
    error_log("OCR Result Final: " . json_encode($result));
    return $result;
}
